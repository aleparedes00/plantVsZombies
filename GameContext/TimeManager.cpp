...
void TimeManager::Update()
{
    ...
    counter += static_cast<unsigned int>(elapsed.count() * 1000.0); // Time is in milliseconds : 1000 = 1 sec
}
...
unsigned int TimeManager::GetElapsedTime()
{
    ...
    {
        std::chrono::duration<double> elapsed = t1 - t2;
        return static_cast<unsigned int>(elapsed.count() * 1000.0); // Time is returned in milliseconds : 1000 = 1 sec
    } else
        return 0;
}
